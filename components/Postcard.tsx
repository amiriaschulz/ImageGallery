...
const Postcard: React.FC<PostcardProps> = ({
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    if (activePointers.current.size === 1) {
      // Start of a drag
      const pointer = Array.from(activePointers.current.values())[0] as any;
      ...
      gestureInfo.current.initialDistance = getDistance(
        pointers[0],
        pointers[1]
      );
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (!dragInfo.current.moved) {
      ...
      const pointer = Array.from(activePointers.current.values())[0] as any;
      const initialX =
        activePointers.current.size === 1 ? dragInfo.current.startX : 0;
      const initialY =
        activePointers.current.size === 1 ? dragInfo.current.startY : 0;
    ...
    if (activePointers.current.size === 1) {
      ...
      const newScale =
        gestureInfo.current.initialTransform.scale *
        (currentDistance / gestureInfo.current.initialDistance);
      const newRot =
        gestureInfo.current.initialTransform.rot +
        (currentAngle - gestureInfo.current.initialAngle);
